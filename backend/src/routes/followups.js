...
router.post('/schedule', async (req, res) => {
  ...
  await addAuditLog('followup.scheduled', { followUpId: followUp.id, applicationId: followUp.applicationId, userId: req.auth.userId })
...
router.patch('/:id', async (req, res) => {
  let followUp = null
  await updateStore((store) => {
    followUp = store.followUps.find(item => item.id === req.params.id && item.userId === req.auth.userId)
    if (!followUp) return
  ...
  await addAuditLog('followup.updated', { followUpId: followUp.id, status: followUp.status, userId: req.auth.userId })
  res.json({ followUp })
})
